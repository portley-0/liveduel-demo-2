...
contract RoundConsumer is FunctionsClient, ConfirmedOwner {
    ...
    mapping(uint256 => bytes) public rawRoundData;

    event RequestedRound(
    ...
    function fulfillRequest(
        ...
        rawRoundData[tid] = response;

        emit RawRoundReady(tid);

        MarketFactory(owner()).onRawDataReady(tid);

    }

    function clearRawRoundData(uint256 tournamentId) external onlyOwner {
        delete rawRoundData[tournamentId];
    }
