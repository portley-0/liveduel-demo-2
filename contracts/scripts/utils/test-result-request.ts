...
async function main() {
  ...
  const requestedResultEvent = receipt.events?.filter((x: EventLog) => x.eventName === "RequestedResult");
  ...
  while (Date.now() - startTime < maxWaitTime) {
    ...
    for (const event of resultReceivedEvents) {
      if ((event as any).args?.matchId.toString() === matchId.toString()) {
        // Always string-ify these fields
        const outcome = (event as any).args.outcome.toString();
        const homeId  = (event as any).args.homeId.toString();
        const awayId  = (event as any).args.awayId.toString();

        console.log("✅ ResultReceived event detected:");
        console.log(`  Match ID:       ${(event as any).args.matchId.toString()}`);
    ...
    if (requestFailedEvents.length > 0) {
      for (const event of requestFailedEvents) {
        if ((event as any).args?.matchId.toString() === matchId.toString()) {
          ...
          console.log(`  Match ID:    ${(event as any).args.matchId.toString()}`);
          console.log(`  Request ID:  ${(event as any).args.requestId}`);
          console.log(`  Error Msg:   ${(event as any).args.errorMessage}`);
        }
      }
