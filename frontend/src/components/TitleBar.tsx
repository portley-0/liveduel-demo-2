      ...
      <div className="drawer-content">
        <header className="flex items-center justify-between px-4 py-2 bg-darkblue h-[84px] shadow-md z-50 select-none">
          <div className="flex items-center space-x-2">
            <label
              htmlFor="my-drawer"
              className="drawer-button bg-transparent border-0 text-white rounded-full hover:bg-gray-200 hover:text-darkblue z-50 flex items-center justify-center w-12 h-12"
            >
              <RiMenuLine className="!text-3xl !sm:text-2xl" />
            </label>
            <NavLink
              to="/"
              className="btn bg-transparent border-none hover:bg-transparent p-0 hover:opacity-80"
            >
              <img
        ...
        {/* Mobile Navigation */}
        <div className="fixed bottom-0 w-full bg-darkblue flex justify-around py-2 shadow-xl z-50 md:hidden">
          {navItems.map(({ path, label, icon: Icon }) => (
            <NavLink
              key={path}
              to={path}
